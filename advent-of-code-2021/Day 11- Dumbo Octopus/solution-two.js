...
function resetFlashed(universe) {
    ...
    let numFlashed = universe.state.flashed.length;
    universe.state.flashed = [];

    universe.state.current++;

    if (numFlashed == universe.data.length * universe.data[0].length) {
        if (!universe.state.allFlashed) {
            universe.state.allFlashed = universe.state.current;
        }
    }
...
const state = { flashed: [], flashCount: 0, current: 0 };
const universe = {
    data: data,
    state: state
};
...
console.log(results.state.allFlashed);